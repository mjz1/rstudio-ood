...
rstudio_session_status <- function(sample_seconds = 1) {
  ellmer::tool(
    ...
    description = paste(
      ...
      "(a PURE sleep, which returns on its own -- no action needed),",
      "waiting-io (a disk transfer), waiting (blocked on something that may",
      "be a wedge -- a timed poll, an indefinite wait or a read; evidence and",
      "advice included), or dead -- WITHOUT touching the session itself (reads",
